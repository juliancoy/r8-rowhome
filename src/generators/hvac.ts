...
export function addStandardHvacSystem(
  ...
  for (let floor = 0; floor < config.stories; floor += 1) {
    ...
    duct(components, {
      ...
      axis: "y",
      from: floor === 0 ? "supply-trunk-1-takeoff" : `supply-trunk-${level}-takeoff`,
    ...
    duct(components, {
      ...
      to: floor === 0 ? "return-trunk-1-drop" : `return-trunk-${level}-collector`,
      flowCfm: 400,
    ...
    if (floor > 0) {
      ...
      duct(components, {
        ...
        axis: "z",
        from: `supply-trunk-${level - 1}-takeoff`,
        to: `supply-trunk-${level}-takeoff`,
      ...
      duct(components, {
        ...
        axis: "z",
        from: `return-trunk-${level}-collector`,
        to: level === 2 ? "return-trunk-1-drop" : `return-trunk-${level - 1}-collector`,
        flowCfm: level === 2 ? 800 : 400,
        designVelocityFpm: 650,
        role: "return"
      });
