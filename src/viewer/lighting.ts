...
export function buildHouseLighting(model: RowhomeModel, intensityMultiplier = 1): Group {
  const lights = new Group();
  lights.name = "House lighting";

  for (const component of model.components) {
    if (overheadLightPattern.test(component.metadata.id)) {
      const light = new PointLight("#ffdca0", 2.1 * intensityMultiplier, 18, 2);
    ...
    if (component.metadata.id === "floor-lamp-bulb") {
      const light = new PointLight("#ffc878", 2.8 * intensityMultiplier, 15, 2);
